# com.viaoa.metadata.pojo.PojoImportMatch

## Purpose

Describes an "import match" strategy for a {@link PojoLinkOne}. An import match defines how a related {@code OAObject} can be located when the JSON does not carry a primary key for the target type. The match can either be: a direct scalar {@link PojoProperty}, or a nested {@link PojoLinkOneReference} that ultimately leads to one or more scalar properties used for matching. Instances of this class are created by {@link OAObjectPojoLoader} based on {@code importMatch} flags on {@code OAPropertyInf

## Architectural Role

PojoImportMatch is a class in the oa metadata model area. Its invariants should be interpreted through the package role: Stores annotation-derived class, property, link, calc, method, and model metadata used by runtime services.
...
### INV-POJOIMPORTMATCH-001: Metadata objects represent model descriptors

**Contract**

PojoImportMatch instances must describe model classes/members and not runtime object state.
...
src/main/java/com/viaoa/metadata/pojo/PojoImportMatch.java, fields and getters/setters
...
### INV-POJOIMPORTMATCH-002: Names and reverse relationships are stable after setup
...
- Source file: `src/main/java/com/viaoa/metadata/pojo/PojoImportMatch.java`
