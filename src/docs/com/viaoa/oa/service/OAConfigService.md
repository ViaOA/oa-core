# com.viaoa.oa.service.OAConfigService

## Purpose

Configuration service for OA runtime-level settings exposed through {@link ConfigOps}.

## Architectural Role

OAConfigService is a class in the oa runtime services area. Its invariants should be interpreted through the package role: Owns model-level runtime services such as config, model user, session user, sync, replication, and triggers.
...
Type declaration relationship: implements ConfigOps.

## Invariants

### INV-OACONFIGSERVICE-001: Service delegates preserve runtime ownership

**Contract**

OAConfigService must operate on OAObject/Hub state through the owning OA runtime and must not create unrelated global ownership.
...
src/main/java/com/viaoa/oa/service/OAConfigService.java, public/protected service methods
...
### INV-OACONFIGSERVICE-002: Null and class context are explicit
...
- Source file: `src/main/java/com/viaoa/oa/service/OAConfigService.java`
- Package: `com.viaoa.oa.service`
- Type kind: `class`
- Declaration relationship: `implements ConfigOps`.
