# com.viaoa.oa.service.OASessionUserService

## Purpose

Stores and resolves the current session user for an OA execution path. Session users represent the application/session actor and are distinct from ModelUser permission identity.

## Architectural Role

OASessionUserService is a class in the oa runtime services area. Its invariants should be interpreted through the package role: Owns model-level runtime services such as config, model user, session user, sync, replication, and triggers.
...
Public/protected methods reviewed: get, set.

Annotations present: Override.

Type declaration relationship: implements SessionUserOps.

## Invariants

### INV-OASESSIONUSERSERVICE-001: Service delegates preserve runtime ownership

**Contract**

OASessionUserService must operate on OAObject/Hub state through the owning OA runtime and must not create unrelated global ownership.
...
src/main/java/com/viaoa/oa/service/OASessionUserService.java, public/protected service methods
...
### INV-OASESSIONUSERSERVICE-002: Null and class context are explicit
...
- Source file: `src/main/java/com/viaoa/oa/service/OASessionUserService.java`
- Package: `com.viaoa.oa.service`
- Type kind: `class`
- Methods/constructors referenced by invariant review: `get`, `set`.
- Declaration relationship: `implements SessionUserOps`.
