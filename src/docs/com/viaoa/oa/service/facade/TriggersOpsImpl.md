# com.viaoa.oa.service.facade.TriggersOpsImpl

## Purpose

Public trigger service facade implementation. This facade delegates trigger registration and removal to the OA trigger service owned by the runtime.

## Architectural Role

TriggersOpsImpl is a class in the oa runtime services area. Its invariants should be interpreted through the package role: Owns model-level runtime services such as config, model user, session user, sync, replication, and triggers.
...
Public/protected methods reviewed: addTrigger, removeTrigger.
...
### INV-TRIGGERSOPSIMPL-001: Service delegates preserve runtime ownership

**Contract**

TriggersOpsImpl must operate on OAObject/Hub state through the owning OA runtime and must not create unrelated global ownership.
...
src/main/java/com/viaoa/oa/service/facade/TriggersOpsImpl.java, public/protected service methods
...
### INV-TRIGGERSOPSIMPL-002: Null and class context are explicit
...
- Source file: `src/main/java/com/viaoa/oa/service/facade/TriggersOpsImpl.java`
- Package: `com.viaoa.oa.service.facade`
- Type kind: `class`
- Methods/constructors referenced by invariant review: `addTrigger`, `removeTrigger`.
