# com.viaoa.oa.service.hub.HubAutoMatchService
...
HubAutoMatchService is a class in the hub runtime services area. Its invariants should be interpreted through the package role: Implements Hub behavior such as AO, add/remove, detail, sharing, select, sort, events, and relationship maintenance.
...
Public/protected methods reviewed: setAutoMatch, getAutoMatch.

Annotations present: SuppressWarnings.

## Invariants

### INV-HUBAUTOMATCHSERVICE-001: Service delegates preserve runtime ownership

**Contract**

HubAutoMatchService must operate on OAObject/Hub state through the owning OA runtime and must not create unrelated global ownership.
...
src/main/java/com/viaoa/oa/service/hub/HubAutoMatchService.java, public/protected service methods
...
### INV-HUBAUTOMATCHSERVICE-002: Null and class context are explicit
...
- Source file: `src/main/java/com/viaoa/oa/service/hub/HubAutoMatchService.java`
- Package: `com.viaoa.oa.service.hub`
- Type kind: `class`
- Methods/constructors referenced by invariant review: `setAutoMatch`, `getAutoMatch`.
