# com.viaoa.oa.service.hub.HubFindService
...
HubFindService is a class in the hub runtime services area. Its invariants should be interpreted through the package role: Implements Hub behavior such as AO, add/remove, detail, sharing, select, sort, events, and relationship maintenance.
...
Public/protected methods reviewed: getRealObject, findFirst, callObjectCacheGet, callHubDataGetObject.

Annotations present: SuppressWarnings.

## Invariants

### INV-HUBFINDSERVICE-001: Service delegates preserve runtime ownership

**Contract**

HubFindService must operate on OAObject/Hub state through the owning OA runtime and must not create unrelated global ownership.
...
src/main/java/com/viaoa/oa/service/hub/HubFindService.java, public/protected service methods
...
### INV-HUBFINDSERVICE-002: Null and class context are explicit
...
getRealObject, findFirst, callObjectCacheGet, callHubDataGetObject
...
- Source file: `src/main/java/com/viaoa/oa/service/hub/HubFindService.java`
- Package: `com.viaoa.oa.service.hub`
- Type kind: `class`
- Methods/constructors referenced by invariant review: `getRealObject`, `findFirst`, `callObjectCacheGet`, `callHubDataGetObject`.
