# com.viaoa.oa.service.hub.HubSequenceService
...
HubSequenceService is a class in the hub runtime services area. Its invariants should be interpreted through the package role: Implements Hub behavior such as AO, add/remove, detail, sharing, select, sort, events, and relationship maintenance.
...
Public/protected methods reviewed: setAutoSequence, getAutoSequence, resequence, callHubCSIsClient, callHubSortCancelSort.

## Invariants

### INV-HUBSEQUENCESERVICE-001: Service delegates preserve runtime ownership

**Contract**

HubSequenceService must operate on OAObject/Hub state through the owning OA runtime and must not create unrelated global ownership.
...
src/main/java/com/viaoa/oa/service/hub/HubSequenceService.java, public/protected service methods
...
### INV-HUBSEQUENCESERVICE-002: Null and class context are explicit
...
setAutoSequence, getAutoSequence, resequence, callHubCSIsClient, callHubSortCancelSort
...
- Source file: `src/main/java/com/viaoa/oa/service/hub/HubSequenceService.java`
- Package: `com.viaoa.oa.service.hub`
- Type kind: `class`
- Methods/constructors referenced by invariant review: `setAutoSequence`, `getAutoSequence`, `resequence`, `callHubCSIsClient`, `callHubSortCancelSort`.
