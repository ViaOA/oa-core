# com.viaoa.runtime.OADataSourceService
...
OADataSourceService is a class in the runtime registry and thread services area. Its invariants should be interpreted through the package role: Locates OA runtimes and owns thread-local, remote-thread, datasource, and thread execution services.
...
Public/protected methods reviewed: register, unregister, getAll, get, setPosition, getPosition.

## Invariants

### INV-OADATASOURCESERVICE-001: Service delegates preserve runtime ownership

**Contract**

OADataSourceService must operate on OAObject/Hub state through the owning OA runtime and must not create unrelated global ownership.
...
src/main/java/com/viaoa/runtime/OADataSourceService.java, public/protected service methods
...
### INV-OADATASOURCESERVICE-002: Null and class context are explicit
...
register, unregister, getAll, get, setPosition, getPosition
...
- Source file: `src/main/java/com/viaoa/runtime/OADataSourceService.java`
- Package: `com.viaoa.runtime`
- Type kind: `class`
- Methods/constructors referenced by invariant review: `register`, `unregister`, `getAll`, `get`, `setPosition`, `getPosition`.
