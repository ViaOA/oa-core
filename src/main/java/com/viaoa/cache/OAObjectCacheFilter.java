 ...
 * match supplied {@link com.viaoa.filter.OAFilter} conditions.  Whenever
...
public class OAObjectCacheFilter<T extends OAObject> implements OAFilter<T> {
    ...
    public OAObjectCacheFilter(Hub<T> hub, OAFilter<T> filter) {
        ...
        oa.internal().objects().cache().addListener(clazz, cacheListener);
    ...
    public void reselectAndRefresh() {
        ...
        boolean b = oa.internal().hubs().data().setLoadingAllData(hub, true);
        ...
        try {
            ...
            hub.setLoading(bWas);
            if (!b) oa.internal().hubs().data().setLoadingAllData(hub, false);
        }
    }
    ...
    public void refresh(final boolean bSetLoading) {
        ...
        try {
            ...
            if (!b) oa.internal().hubs().data().setLoadingAllData(hub, false);
            oa.internal().hubs().events().fireOnNewListEvent(hub, false);
        }
    }
    ...
    protected void setupTrigger() {
        OATriggerListener<T> triggerListener = new OATriggerListener<T>() {
            ...
            public void onTrigger(final T rootObject, final HubEvent hubEvent, final String pathFromRoot) throws Exception {
                ...
                if (rootObject == null) {
                    ...
                    if (trigger != null && (trigger.getUseBackgroundThread() || trigger.getUseBackgroundThreadIfNeeded()) ) {
                                ...
                                if (changeRefresher == null) {
                                    changeRefresher = new OAChangeRefresher() {
                                        ...
                                        protected void process() throws Exception {
                                    		final OA oa = OARuntime.oa(hub);
                                            boolean bWasLoadingAllData = oa.internal().hubs().data().setLoadingAllData(hub, true);
                                            try {
                                                reselectAndRefresh();
                                            }
                                            finally {
                                                if (!hasChanged()) {
                                                    if (!bWasLoadingAllData) oa.internal().hubs().data().setLoadingAllData(hub, false);
                                                }
                                            }
                                        }
                                    };
                		...
                		final OA oa = OARuntime.oa(hub);
                        oa.internal().hubs().data().setLoadingAllData(hub, true, changeRefresher.getThread());
        ...
        if (trigger != null) {
            ...
            oa.internal().triggers().removeTrigger(trigger);
            trigger = null;
        }
    ...
    public void close() {
        ...
        if (cacheListener != null) {
    		...
    		oa.internal().objects().cache().removeListener(clazz, cacheListener);
            cacheListener = null;
        }
    }
