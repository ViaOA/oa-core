...
public class OAObjectCacheFilter<T extends OAObject> implements OAFilter<T> {
    ...
    public void reselectAndRefresh() {
        ...
        boolean b = og.internal().hubs().data().setLoadingAllData(hub, true);
        ...
        try {
            ...
            hub.setLoading(bWas);
            if (!b) og.internal().hubs().data().setLoadingAllData(hub, false);
        }
    }
    ...
    public void refresh(final boolean bSetLoading) {
        ...
        try {
            ...
            if (!b) og.internal().hubs().data().setLoadingAllData(hub, false);
            og.internal().hubs().events().fireOnNewListEvent(hub, false);
        }
    }
            ...
            public void onTrigger(final T rootObject, final HubEvent hubEvent, final String propertyPathFromRoot) throws Exception {
                ...
                if (rootObject == null) {
                    ...
                    if (trigger != null && (trigger.getUseBackgroundThread() || trigger.getUseBackgroundThreadIfNeeded()) ) {
                                ...
                                if (changeRefresher == null) {
                                    changeRefresher = new OAChangeRefresher() {
                                        ...
                                        protected void process() throws Exception {
                                    		final OAGraphInternal og = (OAGraphInternal) OARuntime.graph(hub);
                                            boolean bWasLoadingAllData = og.internal().hubs().data().setLoadingAllData(hub, true);
                                            try {
                                                reselectAndRefresh();
                                            }
                                            finally {
                                                if (!hasChanged()) {
                                                    if (!bWasLoadingAllData) og.internal().hubs().data().setLoadingAllData(hub, false);
                                                }
                                            }
                                        }
                                    };
                		...
                		final OAGraphInternal og = (OAGraphInternal) OARuntime.graph(hub);
                        og.internal().hubs().data().setLoadingAllData(hub, true, changeRefresher.getThread());
