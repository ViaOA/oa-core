...
public class OACompare {
	...
	public static int compare(Object value, Object matchValue, final int decimalPlaces) {
        ...
        if (value instanceof OAObject || value instanceof OAObjectKey || matchValue instanceof OAObject || matchValue instanceof OAObjectKey) {
        	...
        	OAObjectKey ka = oa.internal().objects().key().createObjectKey(value);
    	    OAObjectKey kb = oa.internal().objects().key().createObjectKey(matchValue);
    	  
    	    if (ka == kb) return 0;
    	    if (ka == null) return -1;
    	    if (kb == null) return 1;
    	    
    	    if (oa.internal().objects().key().isForSameOAObject(null, ka, kb)) return 0;
