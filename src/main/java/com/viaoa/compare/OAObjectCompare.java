...
CODEX
 ...
 - Method: compare(Object, Object) / _compareFields(...)
  - Issue: Visitor state is never cleared between top-level compare() calls.
  - Why it is a problem: Reusing one OAObjectCompare instance can return false positives. After comparing the same
    object pair once, a later comparison of the same identities can return true from the visitor map even if fields
    changed after the first comparison.
  - Classification: CODEX/FIXNOW

 - Method: _compareFields(...)
  - Issue: Only getDeclaredFields() for the concrete class are compared; superclass fields are ignored.
  - Why it is a problem: Two subclass instances can compare equal even when inherited state differs. This is
    especially risky for OA object graphs if important state lives in base classes.
  - Classification: CODEX/FIXNOW

 - Method: _compare(...) array branch
  - Issue: Primitive arrays can throw ClassCastException when reporting unmatched left-side entries.
  - Why it is a problem: The code casts objLeft to Object[] for OAArray.indexOf(...). Valid arrays such as int[]
    are handled by Array.get(...) elsewhere, but fail on this path.
  - Classification: CODEX/FIXNOW

 - Method: _compare(...) array branch
  - Issue: null array elements cannot be matched correctly.
  - Why it is a problem: HashMap.remove(null) returns the stored value, which is also null, so [null] vs [null] is
    treated as “not found” even though the elements match.
  - Classification: CODEX/FIXNOW

 - Method: _compare(...) array branch
  - Issue: Duplicate keys are reported but do not directly mark the comparison as failed.
  - Why it is a problem: The unordered array algorithm assumes unique getKey(...) values. Duplicate keys make
    matching ambiguous and can overwrite left-side elements before comparison.
  - Classification: CODEX/CONTRACT

  - Method: _compareFields(...)
  - Issue: Cycle detection tracks object identity pairs but does not enforce graph aliasing/topology equivalence.
  - Why it is a problem: A graph where two left fields reference the same child can compare equal to a graph where
    the two right fields reference separate but equal children. That may be wrong if this class is meant to compare
    object graph structure, not just field values.
  - Classification: CODEX/CONTRACT

 - Method: _compare(...)
  - Issue: Exact class equality rejects subclass/proxy-compatible objects before field comparison.
  - Why it is a problem: OA runtime objects may be enhanced, proxied, or subclassed in some contexts. Exact class
    matching can report unequal even when the logical OA object state is comparable.
  - Classification: CODEX/CONTRACT

  - Method: _compareFields(...)
  - Issue: Reflective access can fail under Java module/access restrictions.
  - Why it is a problem: AccessibleObject.setAccessible(...) can throw runtime access exceptions on strongly
    encapsulated classes. This is less likely for OA-owned classes, but valid non-java.* inputs can fail at
    runtime.
