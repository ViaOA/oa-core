...
Rationale:
The API package is the callable boundary into OAGraph and must not leak internal service coordination details.
Source scope:
...
Rationale:
Public API calls are the external contract into OAGraph; silent success would corrupt caller assumptions and runtime
orchestration.
Source scope:
SyncOps, ReplOps, TriggerOps.
Related CODEX findings:
