...
public class HubDelegate {
	...
	/**
	 * Verifies that the property in obj is unique for all objects in this Hub (not in all of system). <br>
	 * Note: if OAObject and isLoading, then true is always returned. <br>
	 ...
	 * <p>
	 * Example: when Hub.AO is changed, and the "ripple affect" with other Hub changes. A hub that is updated/changed might access another
	 * Hub that has not been updated yet. This is because the thread that makes the change calls the listeners sequentially.
	 */
	 ...
	 * <p>
	 * This will check that a Hub is currently pointing (shared) with the correct master object. Used when Hubs are shared, master/detail,
	 * hubMerger, hubCopy, hubFilter, hubCombined.
	 *
	 * @param thisHub
	 * @param hubNew    if not null and the Hub is not InSync, then hubNew will be shared or populated with the correct current hub.
	...
	/**
	 * created 20110904 Find the hub that controls whether or not this hub is valid. So that it can also be listened to, so that hub isValid
	 * can be recalculated
	 *
	 ...
	 * Hub. public static Hub getAnyLinkHub(final Hub thisHub) { if (thisHub.getLinkHub() != null) return thisHub; // 20120716 OAFilter<Hub>
	 * filter = new OAFilter<Hub>() {
	 *
	 * @Override public boolean isUsed(Hub h) { return (h.dataa == thisHub.dataa && h.getLinkHub() != null); } }; Hub[] hubs =
	 *           HubShareDelegate.getAllSharedHubs(thisHub, filter); //was: Hub[] hubs = HubShareDelegate.getAllSharedHubs(thisHub); for
	 *           (int i=0; i<hubs.length; i++) { if (hubs[i].dataa == thisHub.dataa && hubs[i].getLinkHub() != null) return hubs[i]; }
	 *           return null; }
	 */
