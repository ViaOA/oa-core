...
GRAPH-OWNERSHIP-001 — OA Is The Runtime Ownership Boundary
...
Source scope: OA, OAImpl, OA, direct graph facade methods, parent service access points.
...
Source scope: OAImpl graph verbs, graph routing methods, object/Hub/service delegation boundaries.
Related CODEX findings: OAImpl foreign class/object/Hub verb findings.
...
GRAPH-APIX-001 — OA Public Surface Is The Supported Runtime API
Contract statement: Application-facing graph operations must be exposed through OA; implementation and internal
...
can evolve.
Source scope: OA, OAImpl, OA, graph.api., graph.api.internal..
Related CODEX findings: OA trigger public surface completeness; OA internal API boundary findings.
Suggested unit tests: testPublicGraphApiExposesIntendedRuntimeVerbs(),
testPublicTriggerOperationsReachableThroughOA(), testApplicationCodeDoesNotDependOnInternalGraphApis()
Spec target section: OG Runtime / Public API Boundary

GRAPH-INTERNAL-001 — Internal Graph APIs Are Runtime-Only
Contract statement: OA and com.viaoa.graph.api.internal APIs may be used by graph/runtime services but
...
Source scope: OA, graph.api.internal.*, OAImpl internal delegation.
Related CODEX findings: OA internal ops are not app contract; internal API import-boundary findings.
...
Source scope: OAImpl initialization, OARuntime graph creation/registration, parent service initialization.
Related CODEX findings: OAImpl partial initialization after package scan failure;
...
applications.
Source scope: OAImpl lifecycle/reset/shutdown behavior, service lifecycle boundaries, trigger/live-view/sync
integration.
...
Source scope: OAImpl routing methods, OARuntime graph lookup, direct graph facade routing.
...
Source scope: OAImpl default graph integration, OARuntime graph/default graph behavior.
...
Source scope: OAImpl package/class registration, metadata service boundaries, object/Hub service delegation.
...
Source scope: OAImpl identity/cache delegation, object service boundary, serialization boundary.
...
Source scope: OA Hub facade methods, HubService delegation, direct graph-to-Hub service boundaries.
...
Source scope: OA object facade methods, OAObjectService delegation, direct graph-to-object service boundaries.
...
authority.
Source scope: OAImpl, OAObjectService, HubService, OASyncService, OAReplicationService, OATriggerService.
...
path.
Source scope: OAImpl, OASyncService boundary, OAReplicationService boundary, object/Hub parent service
coordination.
...
Source scope: OAImpl datasource delegation, object and Hub service boundaries, metadata package/class routing.
...
Source scope: graph.context.OAContext, OAUserAccess, OAImpl context integration, runtime context boundaries.
...
Source scope: OAImpl graph boundary methods, parent service delegation, context/sync/trigger/object/Hub
integration.
...
Source scope: OAImpl sync/replication facade methods, OASyncService, OAReplicationService, object/Hub service
boundaries.
...
Source scope: OAImpl serialization boundaries, object/Hub serialization service boundaries.
...
Source scope: OA trigger facade methods, OATriggerService boundary, TriggerOps integration.
...
Source scope: OA/OAImpl convenience/live view APIs and Hub composition boundaries.
Related CODEX findings: OAImpl live view controller lifecycle and convenience live view ownership findings.
...
Source scope: OAImpl orchestration boundaries, parent service delegation, object/Hub/sync/trigger integration.
...
authoritative.
Source scope: OAImpl facade methods, parent service boundaries, direct graph lifecycle operations.
...
Source scope: OAImpl lifecycle/facade methods, parent service orchestration, sync/replication/trigger
integration.
Related CODEX findings: OAImpl partial initialization; OASync partial startup; OATrigger async false-success;
...
Source scope: OAImpl, parent service lifecycle, sync/replication/trigger boundaries.
...
Source scope: OAImpl, graph context, parent service fields, sync/trigger lifecycle state.
...
Source scope: OA, OAImpl, OA boundaries, parent service delegation.
