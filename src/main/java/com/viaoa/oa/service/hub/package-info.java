...
Contract statement: Hub service classes are owned, created, wired, and coordinated by the owning HubService/OA
...
Source scope: HubParentService parent coordinator; all classes in com.viaoa.oa.service.hub; dependency hooks
...
Spec target section: OA Hub Runtime / Service Ownership
...
reused for the lifetime of the owning HubService unless explicit OA runtime reset/shutdown occurs.
...
Spec target section: OA Hub Runtime / Service Lifecycle
...
Spec target section: OA Hub Runtime / Role Semantics

HUB-OWNERSHIP-001 — Hub Operations Use The Owning Graph And Hub State
Contract statement: Hub operations must execute against the intended owning OA model, Hub data structures, metadata,
master/detail state, and object identity scope.
Rationale: Hubs are OA-scoped semantic collections and must not mix objects, metadata, or services from another
OA model.
Source scope: HubParentService, HubDataService, HubRootService, HubMasterService, HubLinkService, HubDetailService,
HubAddRemoveService.
Related CODEX findings: Package-level OA ownership and service orchestration findings.
...
Spec target section: OA Hub Runtime / OA Ownership
...
Rationale: Hub contents drive UI, OA model traversal, save/delete, sync, selection, and event behavior.
...
Spec target section: OA Hub Runtime / Membership Semantics
...
Rationale: Object-to-Hub references are used by lifecycle, events, delete, save, sync, detail Hubs, and OA model
traversal.
...
Spec target section: OA Hub Runtime / Object-Hub Reference Semantics
...
Spec target section: OA Hub Runtime / Membership Uniqueness
...
Spec target section: OA Hub Runtime / Ordering Semantics
...
Spec target section: OA Hub Runtime / Active Object Semantics
...
Spec target section: OA Hub Runtime / Active Object Position Semantics
...
Rationale: Master/detail behavior is core OA OA model and UI semantics.
...
Spec target section: OA Hub Runtime / Master Detail Semantics
...
Rationale: A Hub can represent OA model relationships; vector mutation without link consistency corrupts
...
Spec target section: OA Hub Runtime / Link Relationship Semantics
...
Spec target section: OA Hub Runtime / Shared Hub Semantics
...
Rationale: Filtered/matched Hubs are runtime views used by UI, queries, and OA operations.
...
Spec target section: OA Hub Runtime / Filtered Hub Semantics
...
Spec target section: OA Hub Runtime / Sort Semantics
...
Spec target section: OA Hub Runtime / Select Refresh Semantics

HUB-LOGD-001 — Loaded, Loading, Empty, And Unloaded States Are Distinct
...
Spec target section: OA Hub Runtime / Load State Semantics
...
Spec target section: OA Hub Runtime / Event Publication Semantics
...
Spec target section: OA Hub Runtime / Before Event Semantics
...
Spec target section: OA Hub Runtime / Event Ordering Semantics
...
Rationale: Hub events can trigger additional OA model mutations; leaked guards or unbounded recursion can suppress
...
Spec target section: OA Hub Runtime / Reentrancy Semantics
...
incomplete state.
Rationale: Silent local/server divergence corrupts Hub contents, OA model links, sync, and replication.
...
Spec target section: OA Hub Runtime / Client Server Authority
...
Spec target section: OA Hub Runtime / Sync Hook Semantics
...
Rationale: deleteAll is a OA model mutation spanning Hub and object lifecycle; partial cleanup can silently corrupt the
OA model.
...
Spec target section: OA Hub Runtime / DeleteAll Semantics
...
Rationale: False success makes Hub state unretryable and can create OA model divergence.
...
Spec target section: OA Hub Runtime / Delete Failure Semantics
...
to metadata, OA runtime role, and object save semantics.
Rationale: Hub membership can represent OA model relationships that must be persisted consistently with object state.
...
Spec target section: OA Hub Runtime / Hub Save Semantics
...
Rationale: Serialization must not accidentally mutate runtime OA model state, emit sync/events, or duplicate Hub
membership.
...
Spec target section: OA Hub Runtime / Serialization Semantics

HUB-SERIALIZE-002 — Deserialized Hubs Resolve Membership Deterministically
Contract statement: Hub deserialization/readResolve behavior must reconstruct or resolve Hub membership using OA runtime
...
Spec target section: OA Hub Runtime / Deserialization Semantics
...
Spec target section: OA Hub Runtime / Size And Status Semantics
...
Spec target section: OA Hub Runtime / Find Semantics

HUB-LIFECYCLE-001 — Hub Lifecycle Cleanup Releases Runtime Links And Listeners
Contract statement: Hub lifecycle cleanup must release OA-owned references, listeners, shared data links, detail/
master bindings, and runtime flags according to ownership contract.
Rationale: Stale Hub references can leak OA model/object state and continue receiving events after intended disposal.
...
Spec target section: OA Hub Runtime / Lifecycle Cleanup Semantics
...
Spec target section: OA Hub Runtime / ThreadLocal Semantics
...
Rationale: False success creates silent Hub corruption and unretryable OA model state.
...
Spec target section: OA Hub Runtime / Failure Semantics
...
Spec target section: OA Hub Runtime / Partial Progress Semantics

HUB-DETERMINISM-001 — Observable Hub Mutations Are Deterministic For The Same Runtime State
Contract statement: For the same OA model state, Hub metadata, role, datasource result, listener outcome, and object
...
Source scope: All com.viaoa.oa.service.hub services.
...
Spec target section: OA Hub Runtime / Deterministic Mutation Semantics
