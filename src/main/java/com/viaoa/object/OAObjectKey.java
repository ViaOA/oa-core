...
/**
 * Used to represent the objectId properties of an OAObject. <br>
 ...
 * if guid is equal then objects are equal. If property key is equal, then objects are equal, unless either of the objects being compared is
 * new.
 * <p>
...
public class OAObjectKey implements Serializable, Comparable {
	...
	*/
	...
	/**
	 * create a key for an object that has a parent. Example: an Order that has/owns OrderItems: an OAObjectKey can be created for an
	 * OrderItem: new OAObjectKey(order, 1). This can be used to then find the OrderItem using a OADataSource.getObject(OrderItem.class,
	 * key) or HubController.get(OrderItem.class, key)
	 */
	...
	public boolean exactEquals(OAObjectKey key) {
		...
		if (key.bNew != this.bNew) {
			return false;
		}
		if (key.guid != this.guid) {
			return false;
		}
		return true;
	}
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof OAObjectKey) {
			...
			for (int j = 0; j < x; j++) {
				if (objectIds[j] == ok.objectIds[j]) {
					continue;
				}
				if (objectIds[j] == null || ok.objectIds[j] == null) {
					return false;
				}
				if (!objectIds[j].equals(ok.objectIds[j])) {
					return false;
				}
			}
			return true;
		...
		if (objectIds.length == 1) {
			if (objectIds[0] != null && objectIds[0].equals(obj)) {
				return true;
			}
		}
		return false;
	...
	public int compareTo(Object obj) {
		...
		if (obj instanceof OAObjectKey) {
			...
			if (this.bEmpty) {
				if (!ok.bEmpty) {
					if (this.bNew && !ok.bNew && this.guid == ok.guid) {
						return 0;
					}
					return -1;
				}
				if (this.guid == 0 || ok.guid == 0) {
					return 0;
				}
				if (this.guid == ok.guid) {
					return 0;
				}
				if (this.guid > ok.guid) {
					return 1;
				...
				if (ok.bEmpty) {
					if (ok.bNew && !this.bNew && this.guid == ok.guid) {
						return 0;
					}
					return 1;
				}
