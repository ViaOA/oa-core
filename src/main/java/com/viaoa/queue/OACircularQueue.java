...
CODEX
  ...
  - Method: setSize, addMessageToQueue, _getMessages, cleanupQueue
  - Issue: queueSize == 0 is accepted.
  - Why it is a problem: A zero-size queue creates a zero-length array, then producer/consumer paths use %
    queueSize, causing ArithmeticException.
  - Classification: CODEX/FIXNOW

  - Method: getAmountAvailable
  - Issue: Overrun check uses (posTail + queueSize) <= queueHeadPosition, while _getMessages uses <.
  - Why it is a problem: When the queue is exactly full, e.g. posTail=0, queueSize=100, head=100, all 100 messages
    are still readable, but getAmountAvailable() throws overrun.
  - Classification: CODEX/FIXNOW

 - Method: _getMessages
  - Issue: Overrun check, available amount calculation, and message copying are not performed under one LOCKQueue
    critical section.
  - Why it is a problem: A consumer can pass the overrun check, then a producer can advance and overwrite slots
    before the consumer copies from msgQueue, causing stale/wrong/null messages without an overrun exception.
  - Classification: CODEX/FIXNOW

 - Method: _getMessages
  - Issue: amt is calculated as (queueHeadPosition - 1) - posTail on the initial unsynchronized fast path.
  - Why it is a problem: If exactly one message is available, amt becomes 0, forcing a wait even though data is
    already present. With finite wait, this can delay delivery until timeout or a later producer notify.
  - Classification: CODEX/FIXNOW
  
 - Method: getMessage(long posTail)
  - Issue: Calls getMessages(..., msWaitUntilNotified) and then returns vals[0] without null/empty guard.
  - Why it is a problem: If wait is interrupted or returns with no messages for any reason, this can throw
    NullPointerException instead of returning null or preserving the wait contract.
  - Classification: CODEX/DEFER


 - Method: _getMessages
  - Issue: bWaitingToGet is a single boolean shared by all waiting consumers.
  - Why it is a problem: Multiple sessions can wait concurrently. A producer clears the flag and notifies all; if
    one or more consumers re-enter wait, the boolean no longer represents all waiters. This does not always lose
    notifications because wait uses the lock, but the flag is not a correct waiter count.
  - Classification: CODEX/CONTRACT


 - Method: getMessages(int sessionId, long posTail, ...)
  - Issue: Session queuePos is updated after _getMessages returns and outside LOCKQueue.
  - Why it is a problem: Producer-side overrun/throttle scans session positions under LOCKQueue, but consumers
    mutate queuePos outside that lock. Volatile gives visibility, but the update is not ordered with message
    retrieval and cleanup as one queue-state transition.
  - Classification: CODEX/CONTRACT

 - Method: _addMessage
  - Issue: shouldWaitOnSlowSession() can be invoked while holding LOCKQueue.
  - Why it is a problem: Subclasses can override this hook. If the hook blocks, calls back into this queue, or
    performs synchronization that depends on consumers/producers, it can stall the whole queue while the main lock
    is held.
  - Classification: CODEX/CONTRACT

 - Method: addMessageToQueue
  - Issue: Return value is the physical array index, not the logical queue position.
  - Why it is a problem: JavaDoc says “position where the message was added.” Callers needing ordering/replay
    position could incorrectly use the returned int and break after wraparound.
  - Classification: CODEX/CONTRACT

 - Method: registerSession
  - Issue: Re-registering an existing sessionId silently replaces the session and starts it at current head.
  - Why it is a problem: If a duplicate register happens accidentally, unread messages for that session are skipped
    without overrun or explicit reset signal.
  - Classification: CODEX/CONTRACT

  - Method: protected OACircularQueue()
  - Issue: Generic type discovery casts the type argument directly to Class.
  - Why it is a problem: A subclass using a parameterized message type, type variable, or generic intermediate
    superclass can throw ClassCastException during construction.
  - Classification: CODEX/DEFER

  - Method: _addMessage
  - Issue: Long overflow recovery resets queueHeadPosition to posHead + 1.
  - Why it is a problem: If queueHeadPosition ever overflows, logical positions collapse back to a small value
