...
/**
 * Replication participant that connects to an {@link OAReplicationMaster}.
 * <p>
 * The client captures local sync messages, writes them to its transaction log,
 * sends client-originated messages to the master, receives master messages, and
 * applies them through the local remote sync implementation.
 * </p>
 */
public class OAReplicationClient extends OAReplicationBase {
	...
	/**
	 * Transaction-log file used for durable client replication records.
	 */
	 ...
	 * @param syncServer     local sync server
	...
	public OAReplicationClient(String tlogFileName, String guid, OASyncServer syncServer, String masterHostName, int masterHostPort) {
		...
		LOG.fine(String.format("OAReplicationClient guid=%s, tlogFileName=%s, masterHostName=%s, masterHostPort=%d", guid, tlogFileName, masterHostName, masterHostPort));
	}

	@Override
	/**
	 * Starts the client replication service.
	 * 
	...
	/**
	 * Initializes client transaction-log state and starts the sender thread.
	 * 
	...
	protected void _start() throws Exception {
		...
		Thread t = new Thread(new Runnable() {
			@Override
			/**
			 * Runnable entry point for the client sender thread.
			 */
			public void run() {
	...
	/**
	 * Stops client replication and closes the current master connection.
	 * 
	 * @throws Exception if shutdown fails
	 */
	/**
	 * Clears the client connection reference and stops the connection.
	 * 
	 * @throws Exception if shutdown fails
	 */
	...
	/**
	 * Sends queued client transaction-log records to the master.
	 */
		...
		for (; !bStop;) {
			...
			try {
				...
				if (!bGotSeqFromMaster) {
					...
					/*
					 * add this back when Master keeps track of last know Client GUID
					 * lastSentMasterSeq x = rcc.getRemoteMaster().getLastReceivedMasterSeq(); if (x
					 * > this.lastSentMasterSeq) { this.lastSentMasterSeq = x; }
					 */
	...
	@Override
	/**
	 * Captures a local sync message into the client transaction log.
	 * 
	...
	/**
	 * Controls whether the client should remain disconnected from the master.
	 * 
	 * @param b {@code true} to disconnect and suppress reconnects
	 */
	...
	/**
	 * returns null if connection can not be made, and is set to null if connected
	 * is stopped.
	 */
	protected OAReplClientConnection getReplClientConnection() {
		if (replClientConnection != null && !replClientConnection.isStopped())
			return replClientConnection;
		if (bDisconnectFromMaster)
			return null;
		...
		replClientConnection = new OAReplClientConnection(guid, masterHostName, masterHostPort, lastSentMasterSeq, lastSentClientSeq) {
			/**
			 * Processes a message delivered by the master connection.
			 * 
			...
			/**
			 * Handles a socket exception from the master connection.
			 * 
			...
			/**
			 * Handles master connection closure.
			 * 
	...
	/**
	 * New message from OARepl Master. This will add it to the queue that the
	 * OAReplicationThread
	 * 
	 * @param masterSeq
	 * @param methodName
	...
	/**
	 * Loads existing client transaction-log state from disk.
	 * 
	 * @throws Exception if the log cannot be loaded
	 */
	protected void loadTLogFile() throws Exception {
		...
		for (;; cnt++) {
			...
			if (!OAStr.equals(guid, tlog.getSource())) {
				alTLogToMaster.put(tlog);
				LOG.fine(String.format("%,d) guid=%s, masterSeq=%,d, clientSeq=%,d, methodName=%s", cnt + 1, guid, tlog.getMasterSeq(), tlog.getClientSeq(), tlog.getMethodName()));
			}
		}
		ois.close();
		bis.close();
	...
	/**
	 * Opens the client transaction-log stream for append or creation.
	 * 
	...
	/**
	 * Creates a new client transaction-log file and writes its header.
	 * 
	...
	/**
	 * Writes and syncs a client transaction-log record.
	 * 
