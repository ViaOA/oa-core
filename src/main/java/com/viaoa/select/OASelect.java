...
v4.0
  ...
  Severity: high
   ...
   Severity: medium
    Location: src/main/java/com/viaoa/datasource/OASelect.java:1203, src/main/java/com/viaoa/datasource/
    OASelect.java:1231
    Finding: Class claims thread safety, but lifecycle operations like cancel, closeQuery, _next, and iterator state
    mutation are not uniformly synchronized. finalize() cleanup is also unreliable.
    Why it matters: Long-running selects can race with cancel/close in UI/background usage.

Concern: max is enforced before oaFilter post-filtering for datasource results.

> have codex check again ... there's more
*/
