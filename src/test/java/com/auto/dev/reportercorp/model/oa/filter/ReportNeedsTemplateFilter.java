...
public class ReportNeedsTemplateFilter extends OAObject implements CustomHubFilter<Report> {
	private static final long serialVersionUID = 1L;

	private static Logger LOG = Logger.getLogger(ReportNeedsTemplateFilter.class.getName());
	...
	public ReportNeedsTemplateFilter() {
		this(null, null, false);
	}

	public ReportNeedsTemplateFilter(Hub<Report> hub) {
		this(null, hub, true);
	}

	public ReportNeedsTemplateFilter(Hub<Report> hubMaster, Hub<Report> hub) {
		this(hubMaster, hub, false);
	}

	public ReportNeedsTemplateFilter(Hub<Report> hubMaster, Hub<Report> hubFiltered, boolean bUseObjectCache) {
	...
	public HubFilter<Report> getHubFilter() {
		...
		hubFilter = new HubFilter<Report>(hubMaster, hub) {
			...
			public boolean isUsed(Report report) {
				return ReportNeedsTemplateFilter.this.isUsed(report);
			}
		};
		hubFilter.addDependentProperty(ReportPP.reportTemplates().hasTemplate(), false);
		hubFilter.refresh();
		return hubFilter;
	}
	...
	public OAObjectCacheFilter<Report> getObjectCacheFilter() {
		...
		cacheFilter = new OAObjectCacheFilter<Report>(hub) {
			...
			protected void reselect() {
				ReportNeedsTemplateFilter.this.reselect();
			}
		};
		cacheFilter.addDependentProperty(ReportPP.reportTemplates().hasTemplate(), false);
		cacheFilter.refresh();
		return cacheFilter;
	}
	...
	@Override
	public boolean isUsed(Report report) {
		Hub<ReportTemplate> hubReportTemplates = report.getReportTemplates();
		for (ReportTemplate reportTemplate : hubReportTemplates) {
			if (!reportTemplate.getHasTemplate()) {
				return true;
			}
		}
		return false;
