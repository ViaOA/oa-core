...
@OAClassFilter(name = "AssignedProd", displayName = "Assigned Prod", hasInputParams = false, description = "list of users that Prod Order can be assigned to.")
public class UserAssignedProdFilter extends OAObject implements CustomHubFilter<User> {
    private static final long serialVersionUID = 1L;
    private static Logger LOG = Logger.getLogger(UserAssignedProdFilter.class.getName());

    public static final String PPCode = ":AssignedProd()";
    ...
    public UserAssignedProdFilter() {
        this(null, null, false);
    }
    public UserAssignedProdFilter(Hub<User> hub) {
        this(null, hub, true);
    }
    public UserAssignedProdFilter(Hub<User> hubMaster, Hub<User> hub) {
        this(hubMaster, hub, false);
    }
    public UserAssignedProdFilter(Hub<User> hubMaster, Hub<User> hubFiltered, boolean bUseObjectCache) {
    ...
    public HubFilter<User> getHubFilter() {
        ...
        hubFilter = new HubFilter<User>(hubMaster, hub) {
            ...
            public boolean isUsed(User user) {
                return UserAssignedProdFilter.this.isUsed(user);
            }
        };
        hubFilter.addDependentProperty(UserPP.prodAssignedToList(), false);
        hubFilter.refresh();
        return hubFilter;
    }
    ...
    public OAObjectCacheFilter<User> getObjectCacheFilter() {
        ...
        cacheFilter = new OAObjectCacheFilter<User>(hub) {
            ...
            protected void reselect() {
                UserAssignedProdFilter.this.reselect();
            }
        };
        cacheFilter.addDependentProperty(UserPP.prodAssignedToList(), false);
        cacheFilter.refresh();
        return cacheFilter;
    }
    ...
    @Override
    public boolean isUsed(User user) {
        boolean b = user.getProdAssignedToList();
        return b;
    }
}
