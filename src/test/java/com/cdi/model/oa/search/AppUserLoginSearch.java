...
public class AppUserLoginSearch extends OAObject {
    ...
    @OAProperty(lowerName = "location", maxLength = 50, displayLength = 20, uiColumnLength = 15)
    ...
    @OAProperty(lowerName = "computerName", displayName = "Computer Name", maxLength = 32, displayLength = 20, uiColumnLength = 15)
    ...
    public OASelect<AppUserLogin> getSelect() {
        ...
        else if (OAString.isNotEmpty(this.location)) {
            if (sql.length() > 0) sql += " AND ";
        ...
        else if (OAString.isNotEmpty(this.computerName)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(computerName);
            if (val.indexOf("%") >= 0) {
    ...
    public void appendSelect(final String fromName, final OASelect select) {
        ...
        if (!isNull(P_Id)) {
            if (sql.length() > 0) sql += " AND ";
            if (!isNull(P_Id2) && id != id2) {
                sql += prefix + AppUserLogin.P_Id + " >= ?";
                args = OAArray.add(Object.class, args, getId());
                sql += " AND " + prefix + AppUserLogin.P_Id + " <= ?";
                args = OAArray.add(Object.class, args, getId2());
            }
            else {
                sql += prefix + AppUserLogin.P_Id + " = ?";
        ...
        if (createdUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + AppUserLogin.P_Created + " = null";
        }
        else if (createdUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + AppUserLogin.P_Created + " != null";
            ...
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + AppUserLogin.P_Created + " = ?";
            args = OAArray.add(Object.class, args, this.created);
        }
        if (locationUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + AppUserLogin.P_Location + " = null OR " + prefix + AppUserLogin.P_Location + " == '')";
        }
        else if (locationUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + AppUserLogin.P_Location + " != null";
        }
        else if (OAString.isNotEmpty(this.location)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(location);
            if (val.indexOf("%") >= 0) {
                sql += prefix + AppUserLogin.P_Location + " LIKE ?";
            }
            else {
                sql += prefix + AppUserLogin.P_Location + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (computerNameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + AppUserLogin.P_ComputerName + " = null OR " + prefix + AppUserLogin.P_ComputerName + " == '')";
        }
        else if (computerNameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + AppUserLogin.P_ComputerName + " != null";
        }
        else if (OAString.isNotEmpty(this.computerName)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(computerName);
            if (val.indexOf("%") >= 0) {
                sql += prefix + AppUserLogin.P_ComputerName + " LIKE ?";
            }
            else {
                sql += prefix + AppUserLogin.P_ComputerName + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
    if (OAString.isNotEmpty(this.customQuery)) {
