...
public class ColorCodeSearch extends OAObject {
    ...
    public OAFilter<ColorCode> getCustomFilter() {
        ...
        filterCustomFilter = new OAFilter<ColorCode>() {
            ...
            public boolean isUsed(ColorCode colorCode) {
                ...
                if (b && filterExtraWhere != null) b = filterExtraWhere.isUsed(colorCode);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(ColorCode searchColorCode) {
        if (!isNull(P_Id)) {
            if (!OACompare.isEqual(searchColorCode.getId(), id)) return false;
        }
        if (codeUseNull) {
            if (OACompare.isNotEmpty(searchColorCode.getCode())) return false;
        }
        else if (codeUseNotNull) {
            if (OACompare.isEmpty(searchColorCode.getCode())) return false;
        }
        else if (code != null) {
            String s = getCode();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchColorCode.getCode(), s)) return false;
        }
        if (nameUseNull) {
            if (OACompare.isNotEmpty(searchColorCode.getName())) return false;
        }
        else if (nameUseNotNull) {
            if (OACompare.isEmpty(searchColorCode.getName())) return false;
        }
        else if (name != null) {
            String s = getName();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchColorCode.getName(), s)) return false;
        }
        return true;
    }
