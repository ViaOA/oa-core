...
public class CountrySearch extends OAObject {
    private static final long serialVersionUID = 1L;
    private static Logger LOG = Logger.getLogger(CountrySearch.class.getName());
    ...
    protected OAFilter<Country> filterExtraWhere;

    public void setExtraWhere(String s, Object ... args) {
        this.extraWhere = s;
        this.extraWhereParams = args;
        if (!OAString.isEmpty(s) && getExtraWhereFilter() == null) {
            OAFilter<Country> f = new OAQueryFilter<Country>(Country.class, s, args);
            setExtraWhereFilter(f);
        }
    }
    public void setExtraWhereFilter(OAFilter<Country> filter) {
        this.filterExtraWhere = filter;
    }
    public OAFilter<Country> getExtraWhereFilter() {
        return this.filterExtraWhere;
    }

    public OASelect<Country> getSelect() {
        String sql = "";
        ...
        switch (getSortByType()) {
            ...
            case SORTBYTYPE_code: sortOrder = Country.P_Code; break;
            case SORTBYTYPE_name: sortOrder = Country.P_Name; break;
        }
        ...
        if (codeUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + Country.P_Code + " = null OR " + Country.P_Code + " == '')";
        }
        else if (codeUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += Country.P_Code + " != null";
        }
        ...
        if (nameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + Country.P_Name + " = null OR " + Country.P_Name + " == '')";
        }
        else if (nameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += Country.P_Name + " != null";
        }
        ...
        OASelect<Country> select = new OASelect<Country>(Country.class, sql, args, sortOrder);
    ...
    private OAFilter<Country> filterDataSourceFilter;
    public OAFilter<Country> getDataSourceFilter() {
        if (filterDataSourceFilter != null) return filterDataSourceFilter;
        filterDataSourceFilter = new OAFilter<Country>() {
            @Override
            public boolean isUsed(Country country) {
                return CountrySearch.this.isUsedForDataSourceFilter(country);
            }
        };
        return filterDataSourceFilter;
    }
    
    private OAFilter<Country> filterCustomFilter;
    public OAFilter<Country> getCustomFilter() {
        if (filterCustomFilter != null) return filterCustomFilter;
        filterCustomFilter = new OAFilter<Country>() {
            @Override
            public boolean isUsed(Country country) {
                boolean b = CountrySearch.this.isUsedForCustomFilter(country);
                if (b && filterExtraWhere != null) b = filterExtraWhere.isUsed(country);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(Country searchCountry) {
        if (!isNull(P_Id)) {
            if (!OACompare.isEqual(searchCountry.getId(), id)) return false;
        }
        if (codeUseNull) {
            if (OACompare.isNotEmpty(searchCountry.getCode())) return false;
        }
        else if (codeUseNotNull) {
            if (OACompare.isEmpty(searchCountry.getCode())) return false;
        }
        else if (code != null) {
            String s = getCode();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchCountry.getCode(), s)) return false;
        }
        if (nameUseNull) {
            if (OACompare.isNotEmpty(searchCountry.getName())) return false;
        }
        else if (nameUseNotNull) {
            if (OACompare.isEmpty(searchCountry.getName())) return false;
        }
        else if (name != null) {
            String s = getName();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchCountry.getName(), s)) return false;
        }
        return true;
    }
    public boolean isUsedForCustomFilter(Country searchCountry) {
        return true;
    }
}
