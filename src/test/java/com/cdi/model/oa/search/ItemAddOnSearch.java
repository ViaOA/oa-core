...
public class ItemAddOnSearch extends OAObject {
    ...
    public void appendSelect(final String fromName, final OASelect select) {
        ...
        if (!isNull(P_Id)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + ItemAddOn.P_Id + " = ?";
            args = OAArray.add(Object.class, args, this.id);
        }
        if (codeUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + ItemAddOn.P_Code + " = null OR " + prefix + ItemAddOn.P_Code + " == '')";
        }
        else if (codeUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + ItemAddOn.P_Code + " != null";
        }
        else if (OAString.isNotEmpty(this.code)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(code);
            if (val.indexOf("%") >= 0) {
                sql += prefix + ItemAddOn.P_Code + " LIKE ?";
            }
            else {
                sql += prefix + ItemAddOn.P_Code + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (nameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + ItemAddOn.P_Name + " = null OR " + prefix + ItemAddOn.P_Name + " == '')";
        }
        else if (nameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + ItemAddOn.P_Name + " != null";
        }
        else if (OAString.isNotEmpty(this.name)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(name);
            if (val.indexOf("%") >= 0) {
                sql += prefix + ItemAddOn.P_Name + " LIKE ?";
            }
            else {
                sql += prefix + ItemAddOn.P_Name + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (priceUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + ItemAddOn.P_Price + " = null";
        }
        else if (priceUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + ItemAddOn.P_Price + " != null";
            ...
            if (sql.length() > 0) sql += " AND ";
            if (!isNull(P_Price2) && price != price2) {
                sql += prefix + ItemAddOn.P_Price + " >= ?";
                args = OAArray.add(Object.class, args, getPrice());
                sql += " AND " + prefix + ItemAddOn.P_Price + " <= ?";
                args = OAArray.add(Object.class, args, getPrice2());
            }
            else {
                sql += prefix + ItemAddOn.P_Price + " = ?";
        ...
        if (discontinuedDateUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + ItemAddOn.P_DiscontinuedDate + " = null";
        }
        else if (discontinuedDateUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + ItemAddOn.P_DiscontinuedDate + " != null";
            ...
            if (sql.length() > 0) sql += " AND ";
            if (discontinuedDate2 != null && !discontinuedDate.equals(discontinuedDate2)) {
                sql += prefix + ItemAddOn.P_DiscontinuedDate + " >= ?";
                args = OAArray.add(Object.class, args, this.discontinuedDate);
                sql += " AND " + prefix + ItemAddOn.P_DiscontinuedDate + " <= ?";
                args = OAArray.add(Object.class, args, this.discontinuedDate2);
            }
            else {
                sql += prefix + ItemAddOn.P_DiscontinuedDate + " = ?";
        ...
        if (descriptionUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + ItemAddOn.P_Description + " = null OR " + prefix + ItemAddOn.P_Description + " == '')";
        }
        else if (descriptionUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + ItemAddOn.P_Description + " != null";
        }
        else if (OAString.isNotEmpty(this.description)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(description);
            if (val.indexOf("%") >= 0) {
                sql += prefix + ItemAddOn.P_Description + " LIKE ?";
            }
            else {
                sql += prefix + ItemAddOn.P_Description + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
    if (OAString.isNotEmpty(this.customQuery)) {
