...
public class ItemCategorySearch extends OAObject {
    ...
    public OAFilter<ItemCategory> getCustomFilter() {
        ...
        filterCustomFilter = new OAFilter<ItemCategory>() {
            ...
            public boolean isUsed(ItemCategory itemCategory) {
                ...
                if (b && filterExtraWhere != null) b = filterExtraWhere.isUsed(itemCategory);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(ItemCategory searchItemCategory) {
        if (!isNull(P_Id)) {
            if (!OACompare.isEqual(searchItemCategory.getId(), id)) return false;
        }
        if (nameUseNull) {
            if (OACompare.isNotEmpty(searchItemCategory.getName())) return false;
        }
        else if (nameUseNotNull) {
            if (OACompare.isEmpty(searchItemCategory.getName())) return false;
        }
        else if (name != null) {
            String s = getName();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchItemCategory.getName(), s)) return false;
        }
        if (descriptionUseNull) {
            if (OACompare.isNotEmpty(searchItemCategory.getDescription())) return false;
        }
        else if (descriptionUseNotNull) {
            if (OACompare.isEmpty(searchItemCategory.getDescription())) return false;
        }
        else if (description != null) {
            String s = getDescription();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchItemCategory.getDescription(), s)) return false;
        }
        return true;
    }
