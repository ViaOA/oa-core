...
public class ItemSearch extends OAObject {
    ...
    */
    ...
    public OASelect<Item> getSelect() {
        ...
        if (OAString.isNotEmpty(this.location)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(location);
            if (val.indexOf("%") >= 0) {
                sql += Item.P_Location + " LIKE ?";
            }
            else {
                sql += Item.P_Location + " = ?";
            }
        ...
        else if (OAString.isNotEmpty(this.qbListId)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(qbListId);
            if (val.indexOf("%") >= 0) {
    ...
    public void appendSelect(final String fromName, final OASelect select) {
        ...
        if (!isNull(P_Id)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_Id + " = ?";
            args = OAArray.add(Object.class, args, this.id);
        }
        if (codeUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + Item.P_Code + " = null OR " + prefix + Item.P_Code + " == '')";
        }
        else if (codeUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_Code + " != null";
        }
        else if (OAString.isNotEmpty(this.code)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(code);
            if (val.indexOf("%") >= 0) {
                sql += prefix + Item.P_Code + " LIKE ?";
            }
            else {
                sql += prefix + Item.P_Code + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (nameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + Item.P_Name + " = null OR " + prefix + Item.P_Name + " == '')";
        }
        else if (nameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_Name + " != null";
        }
        else if (OAString.isNotEmpty(this.name)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(name);
        ...
        if (OAString.isNotEmpty(this.location)) {
            ...
            String val = OAString.convertToLikeSearch(location);
            if (val.indexOf("%") >= 0) {
                sql += prefix + Item.P_Location + " LIKE ?";
            }
            else {
                sql += prefix + Item.P_Location + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (descriptionUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + Item.P_Description + " = null OR " + prefix + Item.P_Description + " == '')";
        }
        else if (descriptionUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_Description + " != null";
        }
        else if (OAString.isNotEmpty(this.description)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(description);
            if (val.indexOf("%") >= 0) {
                sql += prefix + Item.P_Description + " LIKE ?";
            }
            else {
                sql += prefix + Item.P_Description + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (discontinuedDateUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_DiscontinuedDate + " = null";
        }
        else if (discontinuedDateUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_DiscontinuedDate + " != null";
            ...
            if (sql.length() > 0) sql += " AND ";
            if (discontinuedDate2 != null && !discontinuedDate.equals(discontinuedDate2)) {
                sql += prefix + Item.P_DiscontinuedDate + " >= ?";
                args = OAArray.add(Object.class, args, this.discontinuedDate);
                sql += " AND " + prefix + Item.P_DiscontinuedDate + " <= ?";
                args = OAArray.add(Object.class, args, this.discontinuedDate2);
            }
            else {
                sql += prefix + Item.P_DiscontinuedDate + " = ?";
        ...
        if (!isNull(P_IsProduced)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_IsProduced + " = ?";
            args = OAArray.add(Object.class, args, this.isProduced);
        }
        if (!isNull(P_SpecialOnly)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_SpecialOnly + " = ?";
            args = OAArray.add(Object.class, args, this.specialOnly);
        }
        if (priceUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_Price + " = null";
        }
        else if (priceUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_Price + " != null";
            ...
            if (sql.length() > 0) sql += " AND ";
            if (!isNull(P_Price2) && price != price2) {
                sql += prefix + Item.P_Price + " >= ?";
                args = OAArray.add(Object.class, args, getPrice());
                sql += " AND " + prefix + Item.P_Price + " <= ?";
                args = OAArray.add(Object.class, args, getPrice2());
            }
            else {
                sql += prefix + Item.P_Price + " = ?";
        ...
        if (qbListIdUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + Item.P_QbListId + " = null OR " + prefix + Item.P_QbListId + " == '')";
        }
        else if (qbListIdUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Item.P_QbListId + " != null";
        }
        else if (OAString.isNotEmpty(this.qbListId)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(qbListId);
            if (val.indexOf("%") >= 0) {
                sql += prefix + Item.P_QbListId + " LIKE ?";
            }
            else {
                sql += prefix + Item.P_QbListId + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
    if (OAString.isNotEmpty(this.customQuery)) {
