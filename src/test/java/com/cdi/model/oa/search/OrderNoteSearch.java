...
public class OrderNoteSearch extends OAObject {
    ...
    public void appendSelect(final String fromName, final OASelect select) {
        ...
        if (!isNull(P_Id)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + OrderNote.P_Id + " = ?";
            args = OAArray.add(Object.class, args, this.id);
        }
        if (createdUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + OrderNote.P_Created + " = null";
        }
        else if (createdUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + OrderNote.P_Created + " != null";
            ...
            if (sql.length() > 0) sql += " AND ";
            if (created2 != null && !created.equals(created2)) {
                sql += prefix + OrderNote.P_Created + " >= ?";
                args = OAArray.add(Object.class, args, this.created);
                sql += " AND " + prefix + OrderNote.P_Created + " <= ?";
                args = OAArray.add(Object.class, args, this.created2);
            }
            else {
                sql += prefix + OrderNote.P_Created + " = ?";
        ...
        if (noteUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + OrderNote.P_Note + " = null OR " + prefix + OrderNote.P_Note + " == '')";
        }
        else if (noteUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + OrderNote.P_Note + " != null";
        }
        else if (OAString.isNotEmpty(this.note)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(note);
            if (val.indexOf("%") >= 0) {
                sql += prefix + OrderNote.P_Note + " LIKE ?";
            }
            else {
                sql += prefix + OrderNote.P_Note + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
    if (OAString.isNotEmpty(this.customQuery)) {
