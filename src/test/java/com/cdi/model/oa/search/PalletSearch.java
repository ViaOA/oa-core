...
public class PalletSearch extends OAObject {
    ...
    public OAFilter<Pallet> getCustomFilter() {
        ...
        filterCustomFilter = new OAFilter<Pallet>() {
            ...
            public boolean isUsed(Pallet pallet) {
                ...
                if (b && filterExtraWhere != null) b = filterExtraWhere.isUsed(pallet);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(Pallet searchPallet) {
        if (!isNull(P_Id)) {
            if (!OACompare.isEqual(searchPallet.getId(), id)) return false;
        }
        if (codeUseNull) {
            if (OACompare.isNotEmpty(searchPallet.getCode())) return false;
        }
        else if (codeUseNotNull) {
            if (OACompare.isEmpty(searchPallet.getCode())) return false;
        }
        else if (code != null) {
            String s = getCode();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchPallet.getCode(), s)) return false;
        }
        if (nameUseNull) {
            if (OACompare.isNotEmpty(searchPallet.getName())) return false;
        }
        else if (nameUseNotNull) {
            if (OACompare.isEmpty(searchPallet.getName())) return false;
        }
        else if (name != null) {
            String s = getName();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchPallet.getName(), s)) return false;
        }
        return true;
    }
