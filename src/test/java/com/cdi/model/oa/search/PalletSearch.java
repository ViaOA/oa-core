...
public class PalletSearch extends OAObject {
    private static final long serialVersionUID = 1L;

    private static Logger LOG = Logger.getLogger(PalletSearch.class.getName());
    ...
    protected OAFilter<Pallet> filterExtraWhere;

    public void setExtraWhere(String s, Object ... args) {
        this.extraWhere = s;
        this.extraWhereParams = args;
        if (OAString.isNotEmpty(s) && getExtraWhereFilter() == null) {
            OAFilter<Pallet> f = new OAQueryFilter<Pallet>(Pallet.class, s, args);
            setExtraWhereFilter(f);
        }
    }
    public void setExtraWhereFilter(OAFilter<Pallet> filter) {
        this.filterExtraWhere = filter;
    }
    public OAFilter<Pallet> getExtraWhereFilter() {
        return this.filterExtraWhere;
    }

    public OASelect<Pallet> getSelect() {
        final String prefix = "";
        String sql = "";
        ...
        switch (getSortByType()) {
            ...
            case SORTBYTYPE_code: sortOrder = Pallet.P_Code; break;
            case SORTBYTYPE_name: sortOrder = Pallet.P_Name; break;
        }
    ...
    if (OAString.isNotEmpty(this.customQuery)) {
        ...
        OASelect<Pallet> select = new OASelect<Pallet>(Pallet.class, sql, args, sortOrder);
    ...
    public void appendSelect(final String fromName, final OASelect select) {
        ...
        if (!isNull(P_Id)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Pallet.P_Id + " = ?";
            args = OAArray.add(Object.class, args, this.id);
        }
        if (codeUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + Pallet.P_Code + " = null OR " + prefix + Pallet.P_Code + " == '')";
        }
        else if (codeUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Pallet.P_Code + " != null";
        }
        else if (OAString.isNotEmpty(this.code)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(code);
            if (val.indexOf("%") >= 0) {
                sql += prefix + Pallet.P_Code + " LIKE ?";
            }
            else {
                sql += prefix + Pallet.P_Code + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (nameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + Pallet.P_Name + " = null OR " + prefix + Pallet.P_Name + " == '')";
        }
        else if (nameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Pallet.P_Name + " != null";
        }
        else if (OAString.isNotEmpty(this.name)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(name);
            if (val.indexOf("%") >= 0) {
                sql += prefix + Pallet.P_Name + " LIKE ?";
            }
            else {
                sql += prefix + Pallet.P_Name + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
    if (OAString.isNotEmpty(this.customQuery)) {
    ...
    private OAFilter<Pallet> filterDataSourceFilter;
    public OAFilter<Pallet> getDataSourceFilter() {
        if (filterDataSourceFilter != null) return filterDataSourceFilter;
        filterDataSourceFilter = new OAFilter<Pallet>() {
            @Override
            public boolean isUsed(Pallet pallet) {
                return PalletSearch.this.isUsedForDataSourceFilter(pallet);
            }
        };
        return filterDataSourceFilter;
    }
    
    private OAFilter<Pallet> filterCustomFilter;
    public OAFilter<Pallet> getCustomFilter() {
        if (filterCustomFilter != null) return filterCustomFilter;
        filterCustomFilter = new OAFilter<Pallet>() {
            @Override
            public boolean isUsed(Pallet pallet) {
                boolean b = PalletSearch.this.isUsedForCustomFilter(pallet);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(Pallet searchPallet) {
        return true;
    }
    public boolean isUsedForCustomFilter(Pallet searchPallet) {
        return true;
    }
}
