...
public class PhoneSearch extends OAObject {
    ...
    public OASelect<Phone> getSelect() {
        ...
        else if (OAString.isNotEmpty(this.number)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(number);
            if (val.indexOf("%") >= 0) {
    ...
    public void appendSelect(final String fromName, final OASelect select) {
        ...
        if (!isNull(P_Id)) {
            if (sql.length() > 0) sql += " AND ";
            if (!isNull(P_Id2) && id != id2) {
                sql += prefix + Phone.P_Id + " >= ?";
                args = OAArray.add(Object.class, args, getId());
                sql += " AND " + prefix + Phone.P_Id + " <= ?";
                args = OAArray.add(Object.class, args, getId2());
            }
            else {
                sql += prefix + Phone.P_Id + " = ?";
        ...
        if (numberUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + Phone.P_Number + " = null OR " + prefix + Phone.P_Number + " == '')";
        }
        else if (numberUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + Phone.P_Number + " != null";
        }
        else if (OAString.isNotEmpty(this.number)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(number);
            if (val.indexOf("%") >= 0) {
                sql += prefix + Phone.P_Number + " LIKE ?";
            }
            else {
                sql += prefix + Phone.P_Number + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
    if (OAString.isNotEmpty(this.customQuery)) {
