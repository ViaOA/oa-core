...
public class PriceCodeSearch extends OAObject {
    ...
    public void appendSelect(final String fromName, final OASelect select) {
        ...
        if (!isNull(P_Id)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + PriceCode.P_Id + " = ?";
            args = OAArray.add(Object.class, args, this.id);
        }
        if (nameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + PriceCode.P_Name + " = null OR " + prefix + PriceCode.P_Name + " == '')";
        }
        else if (nameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + PriceCode.P_Name + " != null";
        }
        else if (OAString.isNotEmpty(this.name)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(name);
            if (val.indexOf("%") >= 0) {
                sql += prefix + PriceCode.P_Name + " LIKE ?";
            }
            else {
                sql += prefix + PriceCode.P_Name + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (!isNull(P_Rate)) {
            if (sql.length() > 0) sql += " AND ";
            if (!isNull(P_Rate2) && rate != rate2) {
                sql += prefix + PriceCode.P_Rate + " >= ?";
                args = OAArray.add(Object.class, args, getRate());
                sql += " AND " + prefix + PriceCode.P_Rate + " <= ?";
                args = OAArray.add(Object.class, args, getRate2());
            }
            else {
                sql += prefix + PriceCode.P_Rate + " = ?";
        ...
        if (textureUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + PriceCode.P_Texture + " = null OR " + prefix + PriceCode.P_Texture + " == '')";
        }
        else if (textureUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + PriceCode.P_Texture + " != null";
        }
        else if (OAString.isNotEmpty(this.texture)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(texture);
            if (val.indexOf("%") >= 0) {
                sql += prefix + PriceCode.P_Texture + " LIKE ?";
            }
            else {
                sql += prefix + PriceCode.P_Texture + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (colorUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + PriceCode.P_Color + " = null OR " + prefix + PriceCode.P_Color + " == '')";
        }
        else if (colorUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + PriceCode.P_Color + " != null";
        }
        else if (OAString.isNotEmpty(this.color)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(color);
            if (val.indexOf("%") >= 0) {
                sql += prefix + PriceCode.P_Color + " LIKE ?";
            }
            else {
                sql += prefix + PriceCode.P_Color + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
    if (OAString.isNotEmpty(this.customQuery)) {
