...
public class RegionSearch extends OAObject {
    ...
    public OAFilter<Region> getCustomFilter() {
        ...
        filterCustomFilter = new OAFilter<Region>() {
            ...
            public boolean isUsed(Region region) {
                ...
                if (b && filterExtraWhere != null) b = filterExtraWhere.isUsed(region);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(Region searchRegion) {
        if (!isNull(P_Id)) {
            if (!OACompare.isEqual(searchRegion.getId(), id)) return false;
        }
        if (codeUseNull) {
            if (OACompare.isNotEmpty(searchRegion.getCode())) return false;
        }
        else if (codeUseNotNull) {
            if (OACompare.isEmpty(searchRegion.getCode())) return false;
        }
        else if (code != null) {
            String s = getCode();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchRegion.getCode(), s)) return false;
        }
        if (nameUseNull) {
            if (OACompare.isNotEmpty(searchRegion.getName())) return false;
        }
        else if (nameUseNotNull) {
            if (OACompare.isEmpty(searchRegion.getName())) return false;
        }
        else if (name != null) {
            String s = getName();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchRegion.getName(), s)) return false;
        }
        return true;
    }
