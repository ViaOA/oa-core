...
public class SalesOrderStatusSearch extends OAObject {
    ...
    public void appendSelect(final String fromName, final OASelect select) {
        ...
        if (!isNull(P_Id)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + SalesOrderStatus.P_Id + " = ?";
            args = OAArray.add(Object.class, args, this.id);
        }
        if (nameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + SalesOrderStatus.P_Name + " = null OR " + prefix + SalesOrderStatus.P_Name + " == '')";
        }
        else if (nameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + SalesOrderStatus.P_Name + " != null";
        }
        else if (OAString.isNotEmpty(this.name)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(name);
            if (val.indexOf("%") >= 0) {
                sql += prefix + SalesOrderStatus.P_Name + " LIKE ?";
            }
            else {
                sql += prefix + SalesOrderStatus.P_Name + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (descriptionUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + SalesOrderStatus.P_Description + " = null OR " + prefix + SalesOrderStatus.P_Description + " == '')";
        }
        else if (descriptionUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + SalesOrderStatus.P_Description + " != null";
        }
        else if (OAString.isNotEmpty(this.description)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(description);
            if (val.indexOf("%") >= 0) {
                sql += prefix + SalesOrderStatus.P_Description + " LIKE ?";
            }
            else {
                sql += prefix + SalesOrderStatus.P_Description + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
    if (OAString.isNotEmpty(this.customQuery)) {
