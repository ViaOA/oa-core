...
public class TextureSearch extends OAObject {
    private static final long serialVersionUID = 1L;
    private static Logger LOG = Logger.getLogger(TextureSearch.class.getName());
    ...
    protected OAFilter<Texture> filterExtraWhere;

    public void setExtraWhere(String s, Object ... args) {
        this.extraWhere = s;
        this.extraWhereParams = args;
        if (!OAString.isEmpty(s) && getExtraWhereFilter() == null) {
            OAFilter<Texture> f = new OAQueryFilter<Texture>(Texture.class, s, args);
            setExtraWhereFilter(f);
        }
    }
    public void setExtraWhereFilter(OAFilter<Texture> filter) {
        this.filterExtraWhere = filter;
    }
    public OAFilter<Texture> getExtraWhereFilter() {
        return this.filterExtraWhere;
    }

    public OASelect<Texture> getSelect() {
        String sql = "";
        ...
        switch (getSortByType()) {
            ...
            case SORTBYTYPE_code: sortOrder = Texture.P_Code; break;
            case SORTBYTYPE_name: sortOrder = Texture.P_Name; break;
        }
        ...
        if (codeUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + Texture.P_Code + " = null OR " + Texture.P_Code + " == '')";
        }
        else if (codeUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += Texture.P_Code + " != null";
        }
        ...
        if (nameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + Texture.P_Name + " = null OR " + Texture.P_Name + " == '')";
        }
        else if (nameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += Texture.P_Name + " != null";
        }
        ...
        OASelect<Texture> select = new OASelect<Texture>(Texture.class, sql, args, sortOrder);
    ...
    private OAFilter<Texture> filterDataSourceFilter;
    public OAFilter<Texture> getDataSourceFilter() {
        if (filterDataSourceFilter != null) return filterDataSourceFilter;
        filterDataSourceFilter = new OAFilter<Texture>() {
            @Override
            public boolean isUsed(Texture texture) {
                return TextureSearch.this.isUsedForDataSourceFilter(texture);
            }
        };
        return filterDataSourceFilter;
    }
    
    private OAFilter<Texture> filterCustomFilter;
    public OAFilter<Texture> getCustomFilter() {
        if (filterCustomFilter != null) return filterCustomFilter;
        filterCustomFilter = new OAFilter<Texture>() {
            @Override
            public boolean isUsed(Texture texture) {
                boolean b = TextureSearch.this.isUsedForCustomFilter(texture);
                if (b && filterExtraWhere != null) b = filterExtraWhere.isUsed(texture);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(Texture searchTexture) {
        if (!isNull(P_Id)) {
            if (!OACompare.isEqual(searchTexture.getId(), id)) return false;
        }
        if (codeUseNull) {
            if (OACompare.isNotEmpty(searchTexture.getCode())) return false;
        }
        else if (codeUseNotNull) {
            if (OACompare.isEmpty(searchTexture.getCode())) return false;
        }
        else if (code != null) {
            String s = getCode();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchTexture.getCode(), s)) return false;
        }
        if (nameUseNull) {
            if (OACompare.isNotEmpty(searchTexture.getName())) return false;
        }
        else if (nameUseNotNull) {
            if (OACompare.isEmpty(searchTexture.getName())) return false;
        }
        else if (name != null) {
            String s = getName();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchTexture.getName(), s)) return false;
        }
        return true;
    }
    public boolean isUsedForCustomFilter(Texture searchTexture) {
        return true;
    }
}
