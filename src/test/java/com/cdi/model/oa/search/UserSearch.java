...
public class UserSearch extends OAObject {
    ...
    @OAProperty(lowerName = "salesAccess", displayName = "Sales Access", displayLength = 6, uiColumnLength = 12, isNameValue = true)
    ...
    public String getSalesAccessString() {
        User.SalesAccess salesAccess = getSalesAccessEnum();
    ...
    public void setSalesAccessString(String val) {
        ...
        if (OAString.isNotEmpty(val)) {
            User.SalesAccess salesAccess = User.SalesAccess.valueOf(val);
            if (salesAccess != null) x = salesAccess.ordinal();
        }
        if (x < 0) setNull(P_SalesAccess);
        else setSalesAccess(x);
    }

    public User.SalesAccess getSalesAccessEnum() {
        if (isNull(P_SalesAccess)) return null;
        final int val = getSalesAccess();
        if (val < 0 || val >= User.SalesAccess.values().length) return null;
        return User.SalesAccess.values()[val];
    }

    public void setSalesAccessEnum(User.SalesAccess val) {
        if (val == null) {
    ...
    @OAProperty(lowerName = "productionAccess", displayName = "Production Access", displayLength = 6, uiColumnLength = 11, uiColumnName = "Prod Access", isNameValue = true)
    ...
    public String getProductionAccessString() {
        User.ProductionAccess productionAccess = getProductionAccessEnum();
    ...
    public void setProductionAccessString(String val) {
        int x = -1;
        if (OAString.isNotEmpty(val)) {
            User.ProductionAccess productionAccess = User.ProductionAccess.valueOf(val);
            if (productionAccess != null) x = productionAccess.ordinal();
        }
        if (x < 0) setNull(P_ProductionAccess);
        else setProductionAccess(x);
    }

    public User.ProductionAccess getProductionAccessEnum() {
        if (isNull(P_ProductionAccess)) return null;
        final int val = getProductionAccess();
        if (val < 0 || val >= User.ProductionAccess.values().length) return null;
        return User.ProductionAccess.values()[val];
    }

    public void setProductionAccessEnum(User.ProductionAccess val) {
        if (val == null) {
    ...
    public OASelect<User> getSelect() {
        ...
        else if (OAString.isNotEmpty(this.initials)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(initials);
            if (val.indexOf("%") >= 0) {
        ...
        else if (OAString.isNotEmpty(this.email)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(email);
            if (val.indexOf("%") >= 0) {
        ...
        else if (OAString.isNotEmpty(this.phoneCell)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(phoneCell);
            if (val.indexOf("%") >= 0) {
    ...
    public void appendSelect(final String fromName, final OASelect select) {
        ...
        if (!isNull(P_Id)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_Id + " = ?";
            args = OAArray.add(Object.class, args, this.id);
        }
        if (lastNameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + User.P_LastName + " = null OR " + prefix + User.P_LastName + " == '')";
        }
        else if (lastNameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_LastName + " != null";
        }
        else if (OAString.isNotEmpty(this.lastName)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(lastName);
            if (val.indexOf("%") >= 0) {
                sql += prefix + User.P_LastName + " LIKE ?";
            }
            else {
                sql += prefix + User.P_LastName + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (firstNameUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + User.P_FirstName + " = null OR " + prefix + User.P_FirstName + " == '')";
        }
        else if (firstNameUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_FirstName + " != null";
        }
        else if (OAString.isNotEmpty(this.firstName)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(firstName);
            if (val.indexOf("%") >= 0) {
                sql += prefix + User.P_FirstName + " LIKE ?";
            }
            else {
                sql += prefix + User.P_FirstName + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (initialsUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + User.P_Initials + " = null OR " + prefix + User.P_Initials + " == '')";
        }
        else if (initialsUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_Initials + " != null";
        }
        else if (OAString.isNotEmpty(this.initials)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(initials);
            if (val.indexOf("%") >= 0) {
                sql += prefix + User.P_Initials + " LIKE ?";
            }
            else {
                sql += prefix + User.P_Initials + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (codeUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + User.P_Code + " = null OR " + prefix + User.P_Code + " == '')";
        }
        else if (codeUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_Code + " != null";
        }
        else if (OAString.isNotEmpty(this.code)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(code);
            if (val.indexOf("%") >= 0) {
                sql += prefix + User.P_Code + " LIKE ?";
            }
            else {
                sql += prefix + User.P_Code + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (loginIdUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + User.P_LoginId + " = null OR " + prefix + User.P_LoginId + " == '')";
        }
        else if (loginIdUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_LoginId + " != null";
        }
        else if (OAString.isNotEmpty(this.loginId)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(loginId);
            if (val.indexOf("%") >= 0) {
                sql += prefix + User.P_LoginId + " LIKE ?";
            }
            else {
                sql += prefix + User.P_LoginId + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (emailUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + User.P_Email + " = null OR " + prefix + User.P_Email + " == '')";
        }
        else if (emailUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_Email + " != null";
        }
        else if (OAString.isNotEmpty(this.email)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(email);
            if (val.indexOf("%") >= 0) {
                sql += prefix + User.P_Email + " LIKE ?";
            }
            else {
                sql += prefix + User.P_Email + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (phoneCellUseNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += "(" + prefix + User.P_PhoneCell + " = null OR " + prefix + User.P_PhoneCell + " == '')";
        }
        else if (phoneCellUseNotNull) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_PhoneCell + " != null";
        }
        else if (OAString.isNotEmpty(this.phoneCell)) {
            if (sql.length() > 0) sql += " AND ";
            String val = OAString.convertToLikeSearch(phoneCell);
            if (val.indexOf("%") >= 0) {
                sql += prefix + User.P_PhoneCell + " LIKE ?";
            }
            else {
                sql += prefix + User.P_PhoneCell + " = ?";
            }
            args = OAArray.add(Object.class, args, val);
        }
        if (!isNull(P_AdminAccess)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_AdminAccess + " = ?";
            args = OAArray.add(Object.class, args, this.adminAccess);
        }
        if (!isNull(P_SalesAccess)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_SalesAccess + " = ?";
            args = OAArray.add(Object.class, args, this.salesAccess);
        }
        if (!isNull(P_ProductionAccess)) {
            if (sql.length() > 0) sql += " AND ";
            sql += prefix + User.P_ProductionAccess + " = ?";
            args = OAArray.add(Object.class, args, this.productionAccess);
        }
    if (OAString.isNotEmpty(this.customQuery)) {
        if (sql.length() > 0) sql += " AND ";
