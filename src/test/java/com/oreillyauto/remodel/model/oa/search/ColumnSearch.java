...
@XmlRootElement(name = "columnSearch")
...
public class ColumnSearch extends OAObject {
    ...
    public static ColumnSearch jaxbCreate() {
        ColumnSearch columnSearch = (ColumnSearch) OAObject.jaxbCreateInstance(ColumnSearch.class);
        if (columnSearch == null) columnSearch = new ColumnSearch();
        return columnSearch;
    }
    ...
    public OAFilter<Column> getCustomFilter() {
        ...
        filterCustomFilter = new OAFilter<Column>() {
            ...
            public boolean isUsed(Column column) {
                ...
                if (b && filterExtraWhere != null) b = filterExtraWhere.isUsed(column);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(Column searchColumn) {
        if (name != null) {
            ...
            if (!OACompare.isLike(searchColumn.getName(), s)) return false;
        }
        if (newName != null) {
            String s = getNewName();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchColumn.getNewName(), s)) return false;
        }
        if (table != null) {
            if (!OACompare.isEqual(searchColumn.getTable(), table)) return false;
        }
        if (created2 != null) {
            if (!OACompare.isEqualOrBetween(searchColumn.getCreated(), created, created2)) return false;
        }
        if (useTableSearch && getTableSearch() != null) {
            Table table = (Table) searchColumn.getProperty(ColumnPP.table().pp);
