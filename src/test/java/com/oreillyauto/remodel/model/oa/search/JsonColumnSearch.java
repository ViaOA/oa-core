...
@XmlRootElement(name = "jsonColumnSearch")
...
public class JsonColumnSearch extends OAObject {
    ...
    public static JsonColumnSearch jaxbCreate() {
        JsonColumnSearch jsonColumnSearch = (JsonColumnSearch) OAObject.jaxbCreateInstance(JsonColumnSearch.class);
        if (jsonColumnSearch == null) jsonColumnSearch = new JsonColumnSearch();
    ...
    public OASelect<JsonColumn> getSelect() {
        ...
        if (!useJavaTypeSearch && getJavaType() != null) {
            ...
            finder = new OAFinder<JavaType, JsonColumn>(getJavaType(), JavaType.P_JsonColumns);
        }
    ...
    public OAFilter<JsonColumn> getCustomFilter() {
        ...
        filterCustomFilter = new OAFilter<JsonColumn>() {
            ...
            public boolean isUsed(JsonColumn jsonColumn) {
                ...
                if (b && filterExtraWhere != null) b = filterExtraWhere.isUsed(jsonColumn);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(JsonColumn searchJsonColumn) {
        if (name != null) {
            ...
            if (!OACompare.isLike(searchJsonColumn.getName(), s)) return false;
        }
        if (jsonObject != null) {
            if (!OACompare.isEqual(searchJsonColumn.getJsonObject(), jsonObject)) return false;
        }
        if (dataType != null) {
            if (!OACompare.isEqual(searchJsonColumn.getDataType(), dataType)) return false;
        }
        if (javaType != null) {
            if (!OACompare.isEqual(searchJsonColumn.getJavaType(), javaType)) return false;
        }
        if (useJsonObjectSearch && getJsonObjectSearch() != null) {
            JsonObject jsonObject = (JsonObject) searchJsonColumn.getProperty(JsonColumnPP.jsonObject().pp);
            if (jsonObject == null) return false;
            if (!getJsonObjectSearch().isUsedForDataSourceFilter(jsonObject)) return false;
        }
        if (useDataTypeSearch && getDataTypeSearch() != null) {
            DataType dataType = (DataType) searchJsonColumn.getProperty(JsonColumnPP.dataType().pp);
            if (dataType == null) return false;
            if (!getDataTypeSearch().isUsedForDataSourceFilter(dataType)) return false;
        }
        if (useJavaTypeSearch && getJavaTypeSearch() != null) {
            JavaType javaType = (JavaType) searchJsonColumn.getProperty(JsonColumnPP.javaType().pp);
            if (javaType == null) return false;
            if (!getJavaTypeSearch().isUsedForDataSourceFilter(javaType)) return false;
        }
        return true;
    }
