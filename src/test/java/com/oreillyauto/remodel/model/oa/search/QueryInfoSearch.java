...
@XmlRootElement(name = "queryInfoSearch")
...
public class QueryInfoSearch extends OAObject {
    ...
    public static QueryInfoSearch jaxbCreate() {
        QueryInfoSearch queryInfoSearch = (QueryInfoSearch) OAObject.jaxbCreateInstance(QueryInfoSearch.class);
        if (queryInfoSearch == null) queryInfoSearch = new QueryInfoSearch();
    ...
    public OASelect<QueryInfo> getSelect() {
        ...
        if (!OAString.isEmpty(this.sqlCode)) {
            if (sql.length() > 0) sql += " AND ";
            String value = this.sqlCode.replace("*", "%");
            if (!value.endsWith("%")) value += "%";
            if (value.indexOf("%") >= 0) {
    ...
    public OAFilter<QueryInfo> getCustomFilter() {
        ...
        filterCustomFilter = new OAFilter<QueryInfo>() {
            ...
            public boolean isUsed(QueryInfo queryInfo) {
                ...
                if (b && filterExtraWhere != null) b = filterExtraWhere.isUsed(queryInfo);
                return b;
            }
        };
    ...
    public boolean isUsedForDataSourceFilter(QueryInfo searchQueryInfo) {
        if (name != null) {
            ...
            if (!OACompare.isLike(searchQueryInfo.getName(), s)) return false;
        }
        if (sqlCode != null) {
            String s = getSqlCode();
            if (s != null && s.indexOf('*') < 0 && s.indexOf('%') < 0) s += '*';
            if (!OACompare.isLike(searchQueryInfo.getSqlCode(), s)) return false;
        }
        return true;
    }
