...
public class HubListenerTest extends OAUnitTest {
	...
	public void listenerTest() {
		...
		for (int i = 0; i < 100; i++) {
			...
			String s = "c" + pos;
			if (!s.equals(server.getHostName())) {
				cChange++;
			}
				...
				public void run() {
					try {
						...
						for (int i = 0; i < maxIterations; i++) {
							...
							if (i % 6 == 0) {
								HubEventDelegate.getListenerCount(hub1);
							}
							if (i % 7 == 0) {
								HubEventDelegate.getListenerCount(hub2);
							}
							if (i % 3 == 0) {
								HubEventDelegate.getListenerCount(hub3);
							}
							if (i % 4 == 0) {
								HubEventDelegate.getListenerCount(hub4);
							}
							if (i % 5 == 0) {
								try {
