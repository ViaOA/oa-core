...
// test package only
import com.viaoa.remote.multiplexer.remote.RemoteBroadcastInterface;
...
public class OARemoteThreadDelegateTest extends OAUnitTest {
	...
	final TestClient[] testClients = new TestClient[3]; // 15
	...
	public void setup() throws Exception {
		...
		remoteBroadcastProxy = (RemoteBroadcastInterface) remoteMultiplexerServer
				.createBroadcast("broadcast", remoteBroadcast, RemoteBroadcastInterface.class, queueName, queueSize);
	...
	public void test() throws Exception {
		...
		for (int i = 0; i < 10 && aiClientRegisterCount.get() != testClients.length; i++) {
			synchronized (lockServer) {
				System.out.println("   server is waiting for all clients to call register(..), total registered="
						+ aiClientRegisterCount.get());
				lockServer.wait(500);
			}
			Thread.sleep(500);
		}
		...
		for (int i = 0; i < 15 && aiClientRegisterCountNoQ.get() != testClients.length; i++) {
			synchronized (lockServerNoQ) {
				System.out.println("   serverNoQ is waiting for all clients to call register(..), total regsitered="
						+ aiClientRegisterCountNoQ.get());
				lockServerNoQ.wait(500);
			}
			Thread.sleep(500);
		}
		for (TestClient tc : testClients) {
			if (tc.remoteClientInterfaceNoQ == null) {
				System.out.println("testClient NoQ not registered ====> id=" + tc.id);
			}
		}
			...
			Thread t = new Thread() {
				public void run() {
					for (TestClient tc : testClients) {
						...
						if ((System.currentTimeMillis() - tsBegin) > ((maxSecondsToRun - 2) * 1000)) {
							break;
						}
					}
				}
		...
		for (int cnt = 1;;) {
			for (TestClient tc : testClients) {
				...
				if (cnt % 500 == 0) {
					if (cnt % 15000 == 0) {
						System.out.println(" " + aiRemoteCount.get());
					} else {
	...
	class TestClient {
		...
		public void run() throws Exception {
			...
			RemoteBroadcastInterface remoteBroadcast = (RemoteBroadcastInterface) remoteMultiplexerClient
					.lookupBroadcast("broadcast", remoteBroadcastImpl);
