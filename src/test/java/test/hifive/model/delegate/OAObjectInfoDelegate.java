package test.hifive.model.delegate;

import com.viaoa.graph.service.OAObjectService;
import com.viaoa.metadata.OAObjectInfo;
import com.viaoa.object.OAObject;
import com.viaoa.runtime.OARuntime;
import test.hifive.model.oa.Ecard;

public class OAObjectInfoDelegate {

	// TEMP for v4.0 phase 4
	
	public static void setPrimitiveNull(OAObject oaObj, String propertyName, boolean bSetToNull) {
		((OAObjectService) OARuntime.graph(Ecard.class).objects()).getOAObjectInfoService().setPrimitiveNull(oaObj, propertyName, bSetToNull);		
	}
	
	public static OAObjectInfo getObjectInfo(Class<?> c) {
		return ((OAObjectService) OARuntime.graph(Ecard.class).objects()).getOAObjectInfoService().getObjectInfo(c);		
	}

	
}
