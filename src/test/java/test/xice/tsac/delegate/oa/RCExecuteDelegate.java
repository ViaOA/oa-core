...
/**
 * Commands to have RCExecute.run(..) to be ran on RemoteRCClient. Client -> Server -> RemoteRCClient -> RCExecuteDelegate.run(ex)
 * 
 * @author vvia
 */
public class RCExecuteDelegate {
	...
	/**
	 * run the command, called by RemoteRCClient to run on a separate instance on the same server as the tsac server.
	 */
	...
	private static void _run(final RCExecute execute) throws Exception {
		if (execute == null) {
			throw new RuntimeException("execute can not be null");
		}
		...
		if (objx != null) {
			execute.setCompleted(new OADateTime());
		}
	}
